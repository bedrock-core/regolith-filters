...
if (settings.gallery === true) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      '',
      "import { Button, Panel, registerCompiledScreen, render, Scroll, Text, type FunctionComponent, type JSX, type PressEvent } from '@bedrock-core/ui';",
      '',
      ...
      '  const Preview = (): JSX.Element => <Panel width={1} height={1} />;',
      '',
  ...
  console.log(`✅ ui-compile: ${GALLERY_NAME} — ${rows.length} preview(s)`);
}
...
if (forms.length > 0) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      ' * Built only when the ui-compile filter runs with `gallery: true`; a build',
