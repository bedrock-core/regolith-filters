...
type SpawnFailure = { stdout?: string; stderr?: string };
...
function run(settings?: unknown): string {
...
function runFails(settings?: unknown): string {
  try {
    run(settings);
  } catch (error) {
...
// JSON read back out of the workspace is deliberately untyped: these assertions are about the
// merge contract, not about a manifest schema the filter itself never imposes.
const read = (rel: string): any => JSON.parse(fs.readFileSync(path.join(root!, rel), 'utf8'));
