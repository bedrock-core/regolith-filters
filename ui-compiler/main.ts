...
//   3. stamps the entity or the block the screen names: the container sized to
//      the layout, and the entity property or block state the runtime reads the
//      layout key from.
//
// A cheap protocol check decides whether a container is ours at all, and a
// layout key — derived from the screen's namespaced name, so addons built apart
// never hand out the same one — decides which screen. A vanilla container fails
// the first check and renders untouched. The addon's router is a file of its
// own; what lands in vanilla's container files is one modification each,
// inserting the addon's root beside every other pack's.
//
...
/** Only the parts of an entity or block file this filter reads and writes. */
interface HostFile {
  'minecraft:entity'?: {
    description: {
      identifier?: string;
...
for (const { screenPath, name, exportName } of entries) {
  ...
  if (bundle.kind === 'chest') {
    ...
    console.log(
      `✅ ui-compiler: ${name} — ${chest.host.kind} container on ${chest.host.type}, `
      + `${drawn} slot(s), ${channels} channel(s), ${size} container slot(s)`,
    );
  } else {
...
// file named after the addon. The hooks are the addon's copies of the vanilla
// container screens — the chest, for a screen an entity hosts, and the
// data-driven container, for one a block hosts — each holding ONE modification
// that inserts that root into the panel. Nothing is defined in a vanilla file:
// a definition there would replace vanilla's and every other pack's, while a
...
for (const [index, hook] of routing.hooks.entries()) {
  ...
  fs.writeFileSync(
    ...
    note(
      'GENERATED by the ui-compiler filter — do not edit.',
      'Inserts this addon\'s compiled-screen root on a vanilla container screen.',
      '',
...
if (routerFile !== undefined) {
  fs.writeFileSync(
    ...
    note(
      'GENERATED by the ui-compiler filter — do not edit.',
      'Gates this addon\'s compiled layouts onto the container screens; vanilla\'s',
      'own container shows when no layout claims it.',
...
// A screen names the entity or the block it opens from; the compiler knows how
// many slots that needs, and the router which key picks it. Stamping both here
// means nobody keeps a container size or a layout key in step with a layout by
// hand — the failure mode of which is a screen that silently draws cells the
// container does not have, or a host that opens the wrong screen.
//
// An entity gets `minecraft:inventory` sized to the layout and an entity
...
/** The workspace definition with this identifier, under one pack directory. */
const findDefinition = (
  files: string[],
  key: 'minecraft:entity' | 'minecraft:block',
  identifier: string,
): string | undefined =>
  files.find(file => (readJsonc(file) as unknown as HostFile)[key]?.description?.identifier === identifier);

/** @type {Map<string, string>} host type → the screen that claimed it */
const hosts = new Map();

for (const screen of compiled) {
  const { kind, type } = screen.host;
  const claimed = hosts.get(type);

  if (claimed !== undefined) {
    console.error(`❌ ui-compiler: ${claimed} and ${screen.name} both name ${kind} "${type}"; one host opens one screen`);
    process.exit(1);
  }

  hosts.set(type, screen.name);

  const dir = kind === 'entity' ? ENTITY_DIR : BLOCK_DIR;
  const match = findDefinition(
    kind === 'entity' ? entityFiles : blockFiles,
    kind === 'entity' ? 'minecraft:entity' : 'minecraft:block',
    type,
  );

  if (!match) {
    console.error(`❌ ui-compiler: ${screen.name} names ${kind} "${type}", which is not in ${dir}`);
    process.exit(1);
  }

  const definition = readJsonc(match) as unknown as HostFile;
  const { layoutProperty, maxLayout, blockSlotLimit } = runtime;
  const entity = definition['minecraft:entity'];
  const block = definition['minecraft:block'];

  if (kind === 'entity' && entity !== undefined) {
    ...
    console.log(
      `   ↳ ${type}: inventory_size ${screen.allocation.size}, ${layoutProperty} = ${screen.layoutId}`,
    );
  ...
  if (block === undefined) {
    console.error(`❌ ui-compiler: ${rel(match)} has no "minecraft:${kind}"`);
    process.exit(1);
  }

  const components = block.components ??= {};

  // Two components the engine refuses to see together, and the pack is where
  // that is decided: a crafting table already IS a block entity of its own kind.
  if (components['minecraft:crafting_table'] !== undefined) {
    console.error(
      `❌ ui-compiler: ${type} declares "minecraft:crafting_table", which cannot be combined with `
      + '"minecraft:block_entity". A compiled screen needs the block entity for its container: '
      + 'drop the crafting table component, or host the screen on an entity.',
    );
    process.exit(1);
  }

  if (screen.allocation.size > blockSlotLimit) {
    console.error(
  ...
  // `minecraft:block_entity` cannot vary per permutation, so it is stamped on
  // the components the block always has rather than on a permutation of it.
  const blockEntity = components['minecraft:block_entity'] ??= {};

  blockEntity.container = { slot_count: screen.allocation.size };
