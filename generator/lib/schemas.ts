...
import { extractTgz } from "./tar.ts";

export const PACKAGE_NAME = "@minecraft/bedrock-schemas";
...
/** Only the parts of the npm registry document this filter reads. */
...
async function resolveVersion(
  version: string,
  { metaCacheFile, maxAgeHours, log }: ResolveOptions,
  ...
  if (!meta) {
    ...
    meta = (await res.json()) as RegistryMetadata;
 ...
 */
export async function ensureSchemaPackage({
  version,
  cacheDir,
  maxAgeHours,
  log,
}: EnsureOptions): Promise<{ version: string; root: string }> {
