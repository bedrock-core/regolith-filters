...
import { ensureSchemaPackage, PACKAGE_NAME } from "./lib/schemas.ts";
...
function mapPackPathToTemp(p: unknown): string | null {
...
function resolvePackDirsFromConfig(): { bp: string; rp: string } {
  try {
    ...
    console.warn("⚠️ Failed to read packs from config.json; using defaults BP/RP:", e instanceof Error ? e.message : e);
    return { bp: "BP", rp: "RP" };
  }
}
...
function assertNoImports(source: string, file: string): void {
...
async function transpileTsToCjs(source: string, file: string): Promise<string> {
  try {
    ...
    throw new Error(`Failed to transpile ${file}: ${e instanceof Error ? e.message : e}`);
  }
}
...
function makeTemplateHelpers() {
  const tag = (value: any, category: string | null): any => {
  ...
  return {
    defineTemplate: (categoryOrData: any, maybeData?: any): any =>
      typeof categoryOrData === "string" ? tag(maybeData, categoryOrData) : categoryOrData,

    defineMany: (...args: any[]): any => {
...
function evaluateModule(cjsCode: string, file: string): any {
  ...
  try {
    ...
    throw new Error(`Failed to evaluate ${file}: ${e instanceof Error ? e.message : e}`);
  }
}

async function processTsFile(file: string): Promise<number> {
  const rel = file;
 ...
 */
function writeProjectFileIfChanged(relPath: string, content: string): boolean {
    ...
    if (processed.length) {
      ...
      for (const f of processed) {
        try {
          ...
          console.warn(`   ⚠ Could not remove ${f}:`, err instanceof Error ? err.message : err);
        }
      }
    }
  } catch (e) {
