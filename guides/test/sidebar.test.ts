...
import { buildLocale, buildManifest } from '../lib/build.ts';
...
function build({ files = FIXTURE, categories = CATEGORIES }: { files?: Map<string, string>; categories?: Map<string, Record<string, any>> } = {}): any {
...
describe('buildManifest', () => {
  it('orders the sidebar by position then name, hiding hidden pages', () => {
    ...
    expect(manifest.tree.map((n: any) => n.id)).toEqual(['intro', 'getting-started', 'faq']);
    expect(manifest.tree.flatMap((n: any) => (n.t === 'page' ? [n.id] : []))).not.toContain('hidden-page');
  });
  ...
  it('carries category label, collapsed, and resolved link', () => {
    ...
    expect(cat.children.map((n: any) => n.id)).toEqual([
      'getting-started/installation',
      'getting-started/first-screen',
    ]);
  ...
  it('warns and ignores broken category links', () => {
    ...
    const cat = manifest.tree.find((n: any) => n.t === 'cat');
    expect(cat.link).toBeUndefined();
    expect(report.warnings.some((w: string) => w.includes('nope'))).toBe(true);
  });
  ...
  it('carries a page icon and a §7-baked description subtitle key', () => {
    ...
    const intro = manifest.tree.find((n: any) => n.id === 'intro');
    ...
    const plain = manifest.tree.find((n: any) => n.id === 'plain');
...
describe('buildManifest access', () => {
  ...
  const page = (...lines: string[]) => ['---', ...lines, '---', '', 'Text.', ''].join('\n');
  ...
  it('marks gated pages and categories, and inherits down a gated category', () => {
    ...
    expect(cat.children.every((n: any) => n.a === 'op')).toBe(true);
  });
  ...
  it('warns and ignores an access value it does not understand', () => {
    ...
    expect(manifest.pages['intro'].a).toBeUndefined();
    expect(manifest.gated).toBeUndefined();
    expect(report.warnings.some((w: string) => w.includes('wizard'))).toBe(true);
  });
});
