 ...
 */
const tsconfigPathsPlugin = (tsconfigPaths: Record<string, string[]>): Plugin => {
 ...
 */
function loadTsConfig(): TsConfigJson {
  try {
 ...
 */
function resolveEntryPoints(tsconfig: TsConfigJson): string[] {
 ...
 */
function cleanupTypeScriptFiles(): void {
  ...
  for (const entry of allEntries) {
    ...
    try {
      ...
      console.warn(`   ⚠ Could not remove ${entry.name}:`, error instanceof Error ? error.message : error);
    }
  }
}
