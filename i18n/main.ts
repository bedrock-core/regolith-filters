...
function problem(scope: string, msg: string): void {
...
function summarize(list: string[], cap = 8): string {
 ...
 */
function clearLangSection(pack: string, locale: string): void {
...
function updateLanguagesJson(pack: string, locales: string[]): void {
 ...
 */
function writeProjectFileIfChanged(relPath: string, content: string): void {
...
async function main(): Promise<void> {
  ...
  /** locale → path → template */
  const ownRaw = new Map<string, Map<string, string>>();
  ...
  /** namespace → merged group; `byLocale` paths carry no namespace prefix. */
  const libGroups = new Map<string, LibGroup>();
  for (const lib of libs) {
    ...
    if (!group) {
      group = { byLocale: new Map(), allPaths: new Set(), typeRefs: [], names: [], defaultTable: new Map() };
    ...
    const typeLocale = files.has(settings.defaultLocale) ? settings.defaultLocale
      : files.has('en_US') ? 'en_US' : ([...files.keys()].sort()[0] ?? settings.defaultLocale);
  ...
  for (const locale of locales) {
    ...
    const localeMap = own.get(locale) ?? new Map<string, string>();
    const { missing, extra } = checkParity(ownDefault, localeMap);
    ...
    for (const { path: p, expected, got } of checkVarParity(ownDefault, localeMap)) {
  ...
  // ── Vanilla ───────────────────────────────────────────────────────────────
  /** locale → 'vanilla.'-prefixed key → string */
  const vanillaEntries = new Map<string, Map<string, string>>();
  let vanillaKeySet = new Set<string>();
  ...
  for (const locale of locales) {
    ...
    const localeOwn = own.get(locale) ?? new Map<string, string>();
