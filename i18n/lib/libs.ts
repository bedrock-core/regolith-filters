...
/** A dependency that declares `bedrockCore.i18n`. */
export interface I18nLib {
  name: string;
  namespace: string;
  dir: string;
  pkgDir: string;
...
export function discoverI18nLibs(projectRoot: string): I18nLib[] {
   ...
   */
  const visit = (fromDir: string, pkg: any, isRoot = false): void => {
    const names = [...new Set([
...
function resolvePackageDir(start: string, name: string): string | undefined {
 ...
 * Keyed locale → absolute file path.
 */
export function libLocaleFiles(lib: { dir: string }): Map<string, string> {
