...
import { templateVars } from './interp.ts';
...
export function flattenResources(
  value: unknown,
  report: { error(scope: string, msg: string): void },
  prefix = '',
  depth = 0,
): Map<string, string> {
 ...
 */
export function checkVarParity(
  defaultMap: Map<string, string>,
  localeMap: Map<string, string>,
): { path: string; expected: string[]; got: string[] }[] {
  const drifted: { path: string; expected: string[]; got: string[] }[] = [];
 ...
 *
 * Returns the base paths missing `_other`.
 */
export function checkPlurals(map: Map<string, string>): string[] {
...
export function pluralBase(path: string): string | undefined {
  const m = PLURAL_RE.exec(path);
