...
import { checkParity, checkPlurals, checkVarParity, flattenResources } from '../lib/tree.ts';

const collect = () => {
  const errors: string[] = [];
  return { errors, report: { error: (scope: string, msg: string) => errors.push(`${scope}: ${msg}`) } };
};
