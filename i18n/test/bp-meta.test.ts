...
function scaffold(resources: string, files: Record<string, string> = {}): string {
...
function run(work: string): string {
...
const read = (work: string, relPath: string): string => fs.readFileSync(path.join(work, relPath), 'utf-8');
const exists = (work: string, relPath: string): boolean => fs.existsSync(path.join(work, relPath));
