...
Each stage's own settings are documented in its own README; the keys are unchanged here. A project
that needs a step of its own between two stages lists the filters one by one in `config.json`
instead, and puts its own filter where it belongs.

## Documentation

https://bedrock-core.drav.dev/docs/filters/core
